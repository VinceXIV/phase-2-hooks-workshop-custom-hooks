...
export function usePokemon(query) {
  ...
  const [{data, errors, status}, setPokemon] = useState({data: null, errors: null, status: 'idle'});
  useEffect(() => {
    ...
    fetch(`https://pokeapi.co/api/v2/pokemon/${query}`)
      ...
      .then(data => {
        setPokemon({data: data, errors: null, status: "fulfilled"})
      })
      .catch(error => {
        setPokemon({data: null, errors: [error], status: "rejected"})
      })
  }, [query]);
